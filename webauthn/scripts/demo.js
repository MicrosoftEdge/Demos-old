(function () {
	...
	const buttonLogonWWinHello = document.getElementById('button-logon-with-windows-hello');
	const textboxAcctName = document.getElementById('idDiv_AcctName');
	const textboxPwd = document.getElementById('input-password');
	const buttonLogonwPwd = document.getElementById('button-sign-in-with-password');
	...
	const addPasswordField = function() {
		// buttonLogon.removeAttribute('hidden');
		// textboxPwd.removeAttribute('hidden');
		// buttonLogonWWinHello.setAttribute('hidden', 'hidden');
		// buttonLogonwPwd.setAttribute('hidden', 'hidden');
		// buttonLogonWoRegister.setAttribute('hidden', 'hidden');
	...
	const hidePasswordField = function () {
		buttonLogon.setAttribute('hidden', 'true');
		textboxPwd.setAttribute('hidden', 'true');
		buttonLogonWoRegister.setAttribute('hidden', 'true');
		buttonLogonWWinHello.setAttribute('hidden', 'false');
		buttonLogonwPwd.setAttribute('hidden', 'false');
	...
	const showSetupWindowsHelloDialog = function (show) {
		if (show) {
			divSetupwWinHello.setAttribute("hidden", "false");
		} else {
			divSetupwWinHello.setAttribute("hidden", "true");
		}
	};
	...
	const addDirectSignIn = function () {
		textboxPwd.setAttribute('hidden', 'false');
		buttonLogonWoRegister.setAttribute('hidden', 'false');
		buttonLogonwPwd.setAttribute('hidden', 'true');
		buttonLogonWWinHello.setAttribute('hidden', 'true');
	...
	const createCredential = function () {
		try {
			...
			// TODO: Add a error message as a div here
			gotoHome();
		}
	...
	const verify = function () {
		...
		const allowList = [{
			/* Only one type is defined in the current spec:
			   http://www.w3.org/TR/2016/WD-webauthn-20160928/ */
			type: 'ScopedCred',
		...
		navigator.authentication.getAssertion(challenge, options)
	...
	document.addEventListener('DOMContentLoaded', function() {
		// If the password field exists, detect whether the register with Windows Hello exists. If so, hide the password
		// field and show the "Sign In with Windows Hello" button.
		if (textboxAcctName) {
			// featureDetect();
			const credentialId = localStorage.getItem('credentialId');
		...
		if (idButton_LogonWoRegister) {
			idButton_LogonWoRegister.addEventListener('click', gotoHome);
		}
